...
class UploadFlowTests(TestCase):
    ...
    def test_upload_run_processes_workbook_and_exports_pack(self):
        ...
        self.assertContains(summary_preview, 'Download PNG')
        self.assertContains(summary_preview, 'visual-preview-image')
        summary_png = self.client.get(reverse('auditpilot:visual_summary_png', args=[run.id]))
        self.assertEqual(summary_png.status_code, 200)
        self.assertEqual(summary_png['Content-Type'], 'image/png')
        self.assertTrue(summary_png.content.startswith(b'\x89PNG\r\n\x1a\n'))
        entity_preview = self.client.get(reverse('auditpilot:visual_entity', args=[run.id, 'SOC']))
        self.assertEqual(entity_preview.status_code, 200)
        self.assertContains(entity_preview, 'SOC visual findings')
        self.assertContains(entity_preview, 'visual-preview-image')
        entity_png = self.client.get(reverse('auditpilot:visual_entity_png', args=[run.id, 'SOC']))
        self.assertEqual(entity_png.status_code, 200)
        self.assertEqual(entity_png['Content-Type'], 'image/png')
        self.assertTrue(entity_png.content.startswith(b'\x89PNG\r\n\x1a\n'))
    ...
    def test_visual_pack_switches_to_baseline_mode_after_second_completed_run(self):
        ...
        first_run = AuditRun.objects.get()
        workbook_two = SimpleUploadedFile('weekly-b.xlsx', build_workbook_bytes(), content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        self.client.post(reverse('auditpilot:upload_run'), {'workbook': workbook_two, 'as_of_label': '2026-W11', 'uploaded_by': 'Analyst'})
        second_run = AuditRun.objects.order_by('-started_at').first()
        summary_preview = self.client.get(reverse('auditpilot:visual_summary', args=[second_run.id]))
        self.assertEqual(summary_preview.status_code, 200)
        self.assertContains(summary_preview, 'vs previous run')
        self.assertContains(summary_preview, f'Run {first_run.id}')
        entity_preview = self.client.get(reverse('auditpilot:visual_entity', args=[second_run.id, 'JGS']))
        self.assertEqual(entity_preview.status_code, 200)
